...
use eframe::egui::{
  Button, CentralPanel, Color32, ComboBox, Context, DragValue, Frame, RichText, ScrollArea, Slider,
  Stroke, Vec2,
};
    ...
    ui.vertical(|ui| {
      ...
      ui.group(|ui| {
        ...
        ui.horizontal(|ui| {
          ...
          ui.label("Resolution:");
          if ui
            .add(DragValue::new(&mut width).speed(1).range(1..=5000))
            .changed()
          {
          ...
          ui.label("x");
          if ui
            .add(DragValue::new(&mut height).speed(1).range(1..=5000))
            .changed()
          {
            app.set_resolution(app.settings.resolution.width, height);
          }

          ui.checkbox(&mut app.settings.always_open, "Always Open");
        });
                ...
                for trigger in available_triggers {
                  ...
                  if ui.checkbox(&mut is_selected, display_name).changed() {
                    if is_selected && !app.settings.trigger_keys.contains(&trigger) {
                      ...
                      if let Some(pos) =
                        app.settings.trigger_keys.iter().position(|x| *x == trigger)
                      {
        ...
        ui.horizontal(|ui| {
          ...
          for i in 0..3 {
            ui.label(labels[i]);
            ui.add(
              DragValue::new(&mut app.settings.target_color[i])
                .speed(1)
                .range(0..=255),
            );
          }
          ...
          Frame::none()
            .fill(Color32::from_rgb(
              app.settings.target_color[0] as u8,
              app.settings.target_color[1] as u8,
              app.settings.target_color[2] as u8,
            ))
      ...
      ui.horizontal(|ui| {
        ...
        if ui
          .add_sized(
            [ui.available_width() / 2.0, 30.0],
            Button::new(RichText::new(button_text).color(Color32::WHITE)).fill(button_color),
          )
          .clicked()
        {
        ...
        if ui
          .add_sized([ui.available_width(), 30.0], reset_button)
          .clicked()
          && !is_default
        {
          app.reset_settings();
